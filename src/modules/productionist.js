...
class Productionist {
   ...
   */
    async finalizeProductionist(){
        //build the grammar file in memory, using the Grammar class
        this.grammar = await this.loadGrammar(`${this.grammarFileLocation}/${this.contentBundle}.grammar`);
        ...
        try{
            this.trie = await this.loadTrie(`${this.grammarFileLocation}/${this.contentBundle}.marisa`);
        }catch(e){
        ...
        this.expressibleMeanings = await this.loadExpressibleMeanings(`${this.grammarFileLocation}/${this.contentBundle}.meanings`);
        try{
            let repetitionPenalties = undefined;
            if(this.reptitionPenaltyMode === undefined){
                if(HAVE_REPETITIONS_FILE_PRESIST_ACROSS_RUNTIME_INSTANCES){
                    ...
                    repetitionPenalties = await this.loader.loadRepetitions(repetitionsFilePath);
                }
            }
