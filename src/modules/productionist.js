...
import Loader from 'loader-gen'; //NOTE this should be replaced with a context-dependent loading scheme
 ...
 * @type {Number}
 */
var REPETITION_PENALTY_MULTIPLIER = 1.0;
/**
 * configuration constant over how well terms should 'rebound' from being repeated
 * @type {Number}
 */
var REPETITION_PENALTY_RECOVERY_RATE = 1.0;
...
class Productionist {
  constructor(contentBundleName, contentBundleDirectory, probabilisticMode, repetitionPenaltyMode, terseMode, verbosity){
    ...
    //NOTE [Port] JavaScript doesn't have key arguments, order matters
    this.grammar = this.loadGrammar(
      `${contentBundleDirectory}/${contentBundleName}.grammar`
    );
    ...
    //NOTE [Port] remember, non-node targets don't have file I/O
    try {
      this.trie = this.loadTrie(
        `${contentBundleDirectory}/${contentBundleName}.marisa`
      );
    }catch(err){
      this.trie = null
    };
    ...
    this.epressibleMeanings = this.loadExpressibleMeanings(
      `${contentBundleDirectory}/${contentBundleName}.meanings`
    );
    ...
    if (repetitionPenaltyMode){
      if (HAVE_REPETITIONS_FILE_PRESIST_ACROSS_RUNTIME_INSTANCES){
        // check to see if a reptitions file has already been generated for this grammar (from a prior
        // generation instance)l reptition files are saved as serialized dictionaries
        try {
          let repetitionsFilePath = contentBundleDirectory.substring(0, contentBundleDirectory.length - 6)
          //NOTE [Port] shifting all file IO to seperate functions.
          this.repetitionPenalties = this.loadRepetitions(repetitionsFilePath);
          if (this.verbosity > 0){
          ...
          for(let symbol of grammarSymbols){
            this.repetitionsPenalties[symbol.toString()] = 1.0; //NOTE [Port] Not entrely sure about this construction right meow.
                                                                // may want to change it to a String() constructor
          }
          if (this.verbosity > 0){
   ...
   * NOTE [Port] we want to split out file IO from main library, pass in a reference to whatever this file loads
   * @param  {String} grammarFileLocation grammar file location
   * @return {Grammar}                     return a new grammar object
   */
  loadGrammar(grammarFileLocation){
    if (this.verbosity > 0){
      console.log("Loading grammar...");
    }
    return new Grammar(grammarFileLocation);
  }
  ...
  loadTrie(trieFileLocation){
    ...
    // NOTE [Port] Not touching ANY of this yet.  Gonna return null, we'll see if
    // we ever use it.
    return undefined;
  }
   ...
   * @return {ExpressibleMeanings}                    A new ExpressibleMeanings object
   */
  // NOTE [Port] we want to split out fileIO from the main library, see others
  loadExpressibleMeanings(expressibleMeaningsFileLocation){
    ...
    //NOTE [Port] a lot of what I'm about to do is file I/O, that uses a csv that isn't a csv?
    //NOTE [Port] assume a tuple that comes from a tsv file

    let triples = Loader.loadExpressiblemeanings(expressibleMeaningsFileLocation);
   ...
   * @return {undefined} function just modifies internal state
   */
  saveRepetitionPenaltiesFile(){
    //NOTE [Port] as with other porting considerations, we want to move this away from the main library
    //NOTE NO FILE IO HERE.
    //NOTE For browserJS this probably looks more like localStorage
    let pathToRepetitionsFile = `${this.contentBundle}.repetitions`;
    //NOTE oh this is where pickle would have been used
    //NOTE right now, I don't want to deal with any of this, so I'm just gonna mark it for fixing later
    throw new Error("SaveRepeitionPenalitiesFile is unimplemented");
  }
